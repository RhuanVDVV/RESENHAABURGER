...
const products: Product[] = [
  { id: 1, name: 'Resenha Clássico', price: 25.90, category: 'Burgers', image: '/burger1.png', description: 'Pão brioche, carne 180g, queijo e molho.' },
  { id: 2, name: 'Mega Bacon', price: 29.90, category: 'Burgers', image: '/burger2.png', description: 'Carne 180g, muito bacon e cheddar.' },
  { id: 3, name: 'Batata Rústica', price: 12.90, category: 'Acomp.', image: '/fries.png', description: 'Batatas com alecrim.' },
...
// MAPA DE OPÇÕES POR CATEGORIA
// Isso resolve o problema da Coca-Cola com picles
...
export default function Menu() {
  ...
  // Função auxiliar para pegar opções baseadas no produto clicado
  ...
  return (
    <div className="min-h-screen bg-brand-light pb-32">
      <header className="bg-white p-4 sticky top-0 z-10 shadow-sm flex justify-center">
        <h1 className="text-xl font-extrabold text-brand-red">Resenha Burger</h1>
      </header>

      <nav className="p-4 overflow-x-auto whitespace-nowrap bg-white border-b border-gray-100">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-colors ${
                activeCat === cat ? 'bg-brand-red text-white shadow-md' : 'bg-gray-100 text-gray-600'
              }`}
            >
              {cat}
            </button>
      ...
      <section className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
        {filtered.map(product => (
          ...
          >
            {/* ... (Imagem e Texto iguais ao anterior) ... */}
            ...
            <div className="flex flex-col flex-grow justify-between">
              ...
              <div className="text-brand-red font-bold mt-2 text-lg">
      ...
      {selectedProduct && (
        <ProductModal 
          ...
          availableOptions={getOptionsForProduct(selectedProduct)} // Passa opções corretas
          onConfirm={(qty, customs, finalPrice) => {
            // Agora passamos o finalPrice (com extras) para o contexto
